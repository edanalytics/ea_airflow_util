...
def s3_to_local(
    ...
    """
    Copies all the files in an S3 path to a local directory, including any nested folders. 
    """ 
    s3_hook = S3Hook(aws_conn_id=s3_conn_id)
    s3_bucket = s3_hook.get_connection(s3_conn_id).schema

    s3_files = s3_hook.list_keys(bucket_name = s3_bucket, prefix = s3_folder)

    if not s3_files:
        print(f'No files found in the S3 path: {s3_folder}')
        raise AirflowSkipException

    for file in s3_files:
        if not file.endswith('/'):
            subfolder = os.path.dirname(file).replace(s3_folder, '')[1:]  # Exracts the filepath of nested files to create the same structure locally
            local_path = os.path.join(local_filepath, subfolder)
...
def delete_from_s3(
    ...
    """
    Deletes all the files in an S3 path, including any nested folders. 
    """ 
    s3_hook = S3Hook(aws_conn_id=s3_conn_id)
    s3_bucket = s3_hook.get_connection(s3_conn_id).schema
