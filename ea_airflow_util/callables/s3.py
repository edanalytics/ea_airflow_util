...
def disk_to_s3(
    ...
    **context
    ...
    s3_bucket = s3_hook.get_connection(s3_conn_id).schema or bucket
    ...
    for root, _, files in os.walk(local_path):
        ...
        for file in files:
            ...
            if os.path.basename(full_local_path).endswith('.gz'):
                try:
                    s3_hook.load_file(
                        full_local_path, key, bucket_name=s3_bucket,
                ...
                try:
                    s3_hook.load_file(full_local_path, key, bucket_name=s3_bucket, replace=True, encrypt=True)
                except Exception as error:
                    logging.error(error)
